...
public class PromotionActivityController {
     ...
     * @return boolean if true the user can gain a wine else not
     * check whether the user has gained a wine in five days;
     */
    ...
    public Object tencentLocation(@Param("latitude") BigDecimal latitude, @Param("longitude") BigDecimal longitude) {
        try {
            ...
            HttpGet get = new HttpGet(new URIBuilder(systemConfig.getTencentApi())
                    .addParameter("location", latitude + "," + longitude)
                    .addParameter("key", systemConfig.getTencentKey()).build());
