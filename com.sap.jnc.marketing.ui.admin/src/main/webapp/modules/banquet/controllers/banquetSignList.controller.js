(function() {
	// Controller
	angular.module('jnc-admin').controller('jnc-admin.banquet.banquetSignList.controller', [
	// Dependencies
	'$scope', '$http', '$constant',
	// Main
	...
	function($scope, $http, $constant) {
		...
		$scope.clearBanquetSignQuery = function() {
		...
		$scope.signBanquetItem = function() {
			if($scope.selectedReportItem==""){
				toastr.error('请最少选择一条记录');
			}else{
				$http.post($scope.apiAdminURL + 'banquets/verifications/applications/signatures',
						$scope.selectedReportItem).success(function(data) {
							toastr.success('已正确处理','签收完成');
